...
func (r *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.* FROM %s tl JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1",
...
func (r *TodoListPostgres) GetById(userId, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.* FROM %s tl JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND tl.id = $2",
		todoListsTable, usersListsTable)
